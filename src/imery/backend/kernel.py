...
class ProvidersManager(DeviceManager, Object):

    def __init__(self, label, providers_path: Optional[str] = None):
        self._tree = None
        self._label = label
    ...
    def get_children_names(self, path: DataPath) -> Result[None]:
        ...
        if not res:
            return Result.error("ProvidersManager: error loading providers", res)
    ...
    def get_metadata(self, path: DataPath) -> Dict[str, Any]:
        ...
        if not res:
            return Result.error(f"ProvidersManager: could not retrieve metadata for path: {path}", res)
        return res
    ...
    def get_metadata_keys(self, path: DataPath) -> Result[list]:
        ...
        if not res:
            return Result.error(f"ProvidersManager: failed to get metadata for {path}", res)
        metadata = res.unwrapped
        if isinstance(metadata, dict):
            return Ok(list(metadata.keys()))
        return Result.error(f"ProvidersManager: metadata is not a dict at {path}")
    ...
    def get(self, path: DataPath) -> Result:
        ...
        if not res:
            return Result.error(f"ProvidersManager: failed to get metadata for {node_path}", res)
        metadata = res.unwrapped
        if isinstance(metadata, dict):
            if key in metadata:
                return Ok(metadata[key])
            return Result.error(f"ProvidersManager: key '{key}' not found in metadata at {node_path}")
        return Result.error(f"ProvidersManager: metadata is not a dict at {node_path}")

    def set(self, path: DataPath, value) -> Result[None]:
        """Set not implemented"""
        return Result.error("ProvidersManager: set: not implemented")

    def add_child(self, path: DataPath, value0) -> Result[None]:
        return Result.error("ProvidersManager: add-child: not implemented")
    ...
    def _ensure_providers_loaded(self) -> Result[None]:
        ...
        if self._providers_path:
            ...
            for path_str in self._providers_path.split(':'):
                path_str = path_str.strip()
                if path_str:
                    provider_dirs.append(Path(path_str))
        else:
            # Default: Get the project root (go up from src/imery/backend to project root)
            src_root = Path(__file__).parent.parent
            provider_dirs.append(src_root / "providers")
        ...
        for providers_dir in provider_dirs:
            ...
            # Scan each subdirectory in providers_dir
            for provider_dir in providers_dir.iterdir():
                if not provider_dir.is_dir():
                    continue

                main_py = provider_dir / "main.py"
                ...
                try:
                    ...
                    spec = importlib.util.spec_from_file_location(
                        f"imery.providers.{provider_name}.main",
                        main_py
                    )
                    ...
                    return Result.error(f"ProvidersManager: _ensure_providers_loaded: Could not load {main_py}", e)
...
class Kernel(DeviceManager, ActionHandler):
    ...
    def __init__(self, dispatcher, providers_path: Optional[str] = None):
        super().__init__()
        ...
        self._tree = {
            ...
            "children": {
                "providers": {
                    "seed-class": ProvidersManager,
                    "seed-constructor-args":
                        {
                        "label": "devices",
                        "providers-path": self._providers_path
                    }
                },
                "settings": {
    ...
    def open(self, path: DataPath, params: Dict) -> Result:
        ...
        # Create asset - ProvidersManager handles both /available and /opened branches
